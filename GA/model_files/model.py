...
def read_topology(q_CRs):
    with open("model_files/topology_files/{}_CRs/T2_{}_links.json".format(q_CRs, q_CRs), 'r') as json_file:
        ...
        with open("model_files/topology_files/{}_CRs/T2_{}_CRss.json".format(q_CRs, q_CRs), 'r') as json_file:
            data = json.load(json_file)
            ...
            for item in json_nodes:
                ...
                crs[CR_id] = cr
        crs[0] = CR(0, 0, 0)
        with open("model_files/topology_files/{}_CRs/T2_{}_paths.json".format(q_CRs, q_CRs), 'r') as json_paths_file:
...
def RU_location(q_CRs):
    rus = {}
    count = 1
    with open("model_files/topology_files/{}_CRs/T2_{}_CRs.json".format(q_CRs, q_CRs), 'r') as json_file:
        data = json.load(json_file)
        json_crs = data["nodes"]
...
if __name__ == '__main__':
    start_all = time.time()
    FO_Stage_1 = run_stage_1(q_CRs=8)
    end_all = time.time()
