...
def read_topology(q_CRs):
    with open('model_files/topology_files/{}_CRs/Murti_T2_{}_links.json'.format(q_CRs, q_CRs)) as json_file:
        ...
        with open('model_files/topology_files/{}_CRs/Murti_T2_{}_CRs.json'.format(q_CRs, q_CRs)) as json_file:
            data = json.load(json_file)
...
def calculate_fitness(croma, VNCs, paths_map, fitness_list):
    centralization = 0
    fs_distribution = {}
    used_crs = []
    count = 0
    ...
    if croma in fitness_list:
        return fitness_list[croma]

    while bit < len(croma):
        p = paths[paths_map[ru][int(croma[bit])]]
        bit += 1
        vnc = VNCs[int(croma[bit])]
        bit += 1
...
def crossover(parent1, parent2, VNCs, paths_map, VNCs_map, population):
    ...
    new_parent1 = validate_cromo(new_parent1, VNCs, paths_map, VNCs_map, population)
    new_parent2 = validate_cromo(new_parent2, VNCs, paths_map, VNCs_map, population)

    return new_parent1, new_parent2


def mutation(parent1, parent2, mutation_param, VNCs, paths_map, VNCs_map, population):
    for i in range(0, len(parent1)):
        if random.randrange(0, 100) < mutation_param + 1:
            ...
            path_range = [j for j in range(0, len(paths_map[ru]))]
            new_p = random.choice(path_range)
            vnc = -1
            while vnc == -1:
                if VNCs_map[paths_map[ru][new_p]]:
                    vnc = random.choice(VNCs_map[paths_map[ru][new_p]])
                else:
                    new_p = random.choice(path_range)

            parent1 = parent1[:i] + str(new_p) + str(vnc) + parent1[i + 2:len(parent1)]
    for i in range(0, len(parent2)):
        if random.randrange(0, 100) < mutation_param + 1:
            ...
            parent2 = parent2[:i] + str(new_p) + str(vnc) + parent2[i + 2:len(parent2)]

    parent1 = validate_cromo(parent1, VNCs, paths_map, VNCs_map, population)
    parent2 = validate_cromo(parent2, VNCs, paths_map, VNCs_map, population)
    return parent1, parent2


def validate_cromo(cromo, VNCs, paths_map, VNCs_map, population):
    ...
    while count < len(cromo):
        p = paths[paths_map[ru][int(cromo[count])]]
        count += 1
        vnc = VNCs[int(cromo[count])]
        count += 1
        ...
        if valid:
            ...
            if len(population) <= 2:
                ...
                path_range = [i for i in range(0, len(paths_map[ru]))]
                new_p = random.choice(path_range)
                vnc = -1
                while vnc == -1:
                    if VNCs_map[paths_map[ru][new_p]]:
                        ...
                        new_p = random.choice(path_range)
                cromo = cromo[0:bit] + str(new_p) + str(vnc) + cromo[bit + 2:len(cromo)]
            else:
...
def my_AG(population_size, number_of_generations, crossover_param, mutation_param, q_CRs, test_number):
    ...
    for p in paths:
        ...
        if paths[p].seq[0] == 0 and paths[p].seq[1] == 0:
            ...
            VNCs_map[paths[p].id].append(8)
        elif paths[p].seq[0] == 0 and paths[p].seq[1] != 0:
            # only NG_RAN(2) and C-RAN is possible
            if VNCs[6].delay_BH >= paths[p].delay_p1 and VNCs[6].delay_MH >= paths[p].delay_p2 and VNCs[6].delay_FH >= \
                    paths[p].delay_p3:
                VNCs_map[paths[p].id].append(6)
            if VNCs[7].delay_BH >= paths[p].delay_p1 and VNCs[7].delay_MH >= paths[p].delay_p2 and VNCs[7].delay_FH >= \
                    paths[p].delay_p3:
                VNCs_map[paths[p].id].append(7)
            if VNCs[9].delay_BH >= paths[p].delay_p1 and VNCs[9].delay_MH >= paths[p].delay_p2 and VNCs[9].delay_FH >= \
                    paths[p].delay_p3:
                VNCs_map[paths[p].id].append(9)
            if VNCs[0].delay_BH >= paths[p].delay_p1 and VNCs[0].delay_MH >= paths[p].delay_p2 and VNCs[0].delay_FH >= \
                    paths[p].delay_p3:
                VNCs_map[paths[p].id].append(0)
        else:
            # only NG_RAN(3) is possible
            if VNCs[1].delay_BH >= paths[p].delay_p1 and VNCs[1].delay_MH >= paths[p].delay_p2 and VNCs[1].delay_FH >= \
                    paths[p].delay_p3:
                VNCs_map[paths[p].id].append(1)
            if VNCs[2].delay_BH >= paths[p].delay_p1 and VNCs[2].delay_MH >= paths[p].delay_p2 and VNCs[2].delay_FH >= \
                    paths[p].delay_p3:
                VNCs_map[paths[p].id].append(2)
            if VNCs[4].delay_BH >= paths[p].delay_p1 and VNCs[4].delay_MH >= paths[p].delay_p2 and VNCs[4].delay_FH >= \
                    paths[p].delay_p3:
                VNCs_map[paths[p].id].append(4)
            if VNCs[5].delay_BH >= paths[p].delay_p1 and VNCs[5].delay_MH >= paths[p].delay_p2 and VNCs[5].delay_FH >= \
                    paths[p].delay_p3:
                VNCs_map[paths[p].id].append(5)
    ...
    for generation in range(0, number_of_generations):
        ...
        # if generation > number_of_generations/10:
        #     mutation_param = mutation_param - (mutation_param_ini/number_of_generations)
        # else:
        #     mutation_param = 5
        ...
        if not generation:
            for i in range(0, population_size):
                new_cromo = ""
                for ru in rus:
                    path_range = [i for i in range(0, len(paths_map[rus[ru].id]))]
                    path_choice = random.choice(path_range)

                    VNC_choice = -1

                    while VNC_choice == -1:
                        if VNCs_map[paths_map[rus[ru].id][path_choice]]:
                            VNC_choice = random.choice(VNCs_map[paths_map[rus[ru].id][path_choice]])
                        else:
                            path_choice = random.choice(path_range)

                    new_cromo = new_cromo + str(path_choice) + str(VNC_choice)

                new_cromo = validate_cromo(new_cromo, VNCs, paths_map, VNCs_map, population)

                new_fit = calculate_fitness(new_cromo, VNCs, paths_map, fitness_list)

                population[generation].append({"id": new_cromo, "fitness": new_fit})

                if new_cromo not in fitness_list:
                    fitness_list[new_cromo] = new_fit
        ...
        while it < len(selected_parents):
            ...
            if random.randrange(0, 100) < crossover_param + 1:
                parent1["id"], parent2["id"] = crossover(parent1["id"], parent2["id"], VNCs, paths_map, VNCs_map, population)

            parent1["id"], parent2["id"] = mutation(parent1["id"], parent2["id"], mutation_param, VNCs, paths_map,
                                                    VNCs_map, population)

            new_fit = calculate_fitness(parent1["id"], VNCs, paths_map, fitness_list)
            population[generation + 1].append({"id": parent1["id"], "fitness": new_fit})

            if parent1["id"] not in fitness_list:
                fitness_list[parent1["id"]] = new_fit

            new_fit = calculate_fitness(parent2["id"], VNCs, paths_map, fitness_list)

            if parent2["id"] not in fitness_list:
                fitness_list[parent2["id"]] = new_fit
    ...
    for q_CRs in [50, 100, 150, 200, 250, 300, 350, 400, 450, 500]:
        ...
        for j in range(1, 6):
            all_population = my_AG(population_size=100,
                                   ...
                                   crossover_param=50,
                                   mutation_param=[2, 2, 1.5, 1.5, 1, 1, 0.5, 0.5, 0, 0, 0, 0, 0, 0, 0],
                                   q_CRs=q_CRs,
                                   test_number=j)
